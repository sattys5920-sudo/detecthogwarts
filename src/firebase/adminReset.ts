...
const DEMO_PREFIXES_TO_CLEAR = [
  ...
  'arcanum-assignment-broadcast-demo',
  'arcanum-final-survey-answers-demo',
  'arcanum-quidditch-demo-',
  'arcanum-forest-demo-',
  'arcanum-forestchat-demo-',
  'arcanum-herbfarm-demo:',
  'arcanum-notebook-demo:',
...
export async function resetContent(): Promise<void> {
  if (isFirebaseConfigured && db) {
    ...
    await Promise.all([
      ...
      deleteCollectionDocs(firestore, 'assignmentBroadcasts'),
      deleteCollectionDocs(firestore, 'finalSurveyAnswers'),
      resetGameRooms(firestore),
      deleteCollectionDocs(firestore, 'herbFarms'),
      deleteCollectionDocs(firestore, 'notebooks'),
