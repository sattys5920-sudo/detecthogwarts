...
export default function NpcDialogue({ npcIcon, script, onClue }: NpcDialogueProps) {
  const [log, setLog] = useState<LogEntry[]>([{ who: 'npc', text: script.greeting }]);
  ...
  function ask(topic: Topic) {
    setLog((l) => [...l, { who: 'me', text: topic.prompt }, { who: 'npc', text: topic.response }]);
  ...
  function askFollowUp(topic: Topic) {
    ...
    setLog((l) => [...l, { who: 'me', text: topic.followUp!.prompt }, { who: 'npc', text: topic.followUp!.response }]);
      ...
      <div className="flex flex-col gap-1.5">
        {script.topics.map((t) => (
          <div key={t.id} className="flex flex-col gap-1.5">
            ...
            >
              {t.prompt} {asked[t.id] && <span className="text-ink-500/40">✓</span>}
            </button>
